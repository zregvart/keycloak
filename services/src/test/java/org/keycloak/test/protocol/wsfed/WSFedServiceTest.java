...
import static org.keycloak.test.common.TestHelpers.*;
...
public class WSFedServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        service = spy(new WSFedService(mockHelper.getRealm(), this.event));
        injectMocks(service);
    }

    private void setPrivateField(WSFedService service, String field, Object value) throws Exception {
        Field f = WSFedService.class.getDeclaredField(field);
    ...
    private void injectMocks(WSFedService service) throws Exception {
        ...
        // Need to explicitly initialze the fields marked with @Context
        setPrivateField(service, "uriInfo", mockHelper.getUriInfo());
        setPrivateField(service, "session", mockHelper.getSession());
        setPrivateField(service, "clientConnection", clientConnection);
        setPrivateField(service, "securityContext", securityContext);
        setPrivateField(service, "providers", providers);
        setPrivateField(service, "headers", headers);
...
// Commented out on account of isDirectGrantsOnly being deprecated
//    @Test
//    public void testClientChecksDirectGrantsOnlyClient() throws Exception {
//        WSFedProtocolParameters params = new WSFedProtocolParameters();
//        params.setWsfed_action(WSFedConstants.WSFED_SIGNIN_ACTION);
//
//        doReturn(false).when(mockHelper.getClient()).isStandardFlowEnabled();
//        assertNotNull(service.clientChecks(mockHelper.getClient(), params));
//        assertErrorPage(mockHelper.getLoginFormsProvider(), Messages.STANDARD_FLOW_DISABLED);
//    }
//
    @Test
...
//        when(clientSession.getNote(ClientSessionModel.ACTION_SIGNATURE)).thenReturn(sessionCode.getCode()); //This is normally set in method but because we are mocked we need to return it
    ...
    public void testHandleLoginRequestRedirectToIdentityProvider() throws Exception {
        ...
        assertTrue(response.getLocation().getPath().endsWith("broker/dummyIdentityProvider/login"));
    }
}