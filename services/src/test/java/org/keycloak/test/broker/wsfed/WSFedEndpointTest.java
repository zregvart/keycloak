...
package org.keycloak.test.broker.wsfed;

import org.keycloak.broker.wsfed.RequestedToken;
...
public class WSFedEndpointTest {
    ...
    private void injectMocks(WSFedEndpoint endpoint) throws Exception {
        setPrivateField(endpoint, "event", this.event);
    ...
    @Test
    public void testHandleSignoutRequestNoIdentityCookie() throws Exception {
        setPrivateField(endpoint, "authMgr", mock(AuthenticationManager.class));
        when(endpoint.authenticateIdentityCookie(mockHelper.getSession(), mockHelper.getRealm())).thenReturn(null);
    ...
    public void testHandleSignoutRequest() throws Exception {
        ...
        when(endpoint.authenticateIdentityCookie(mockHelper.getSession(), mockHelper.getRealm())).thenReturn(authResult);
    ...
    @Test
    public void testHandleSignoutResponseNoIdentityCookie() throws Exception {
        setPrivateField(endpoint, "authMgr", mock(AuthenticationManager.class));
        ...
        when(callback.authenticated(any(BrokeredIdentityContext.class))).
                    thenAnswer(new Answer<Response>() {
                        public Response answer(InvocationOnMock invocation) {
                            BrokeredIdentityContext identity = BrokeredIdentityContext.class.cast(invocation.getArguments()[0]);
    ...
    public void testHasExpiredNotValidYet() throws Exception {
        ...
        rstr.getLifetime().getCreated().add(DatatypeFactory.newInstance().newDuration(false, 0, 0, 0, 1, 0, 0));
        rstr.getLifetime().getExpires().add(DatatypeFactory.newInstance().newDuration(false, 0, 0, 0, 1, 0, 0));
